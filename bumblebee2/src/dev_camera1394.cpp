...
void Camera1394::findBayerFilter(const char* bayer, const char* method)
{
  ...
  if (strcmp(method, "NONE")) {
    if (!DoBayerConversion_)
      ROS_ERROR("bayer method [%s] specified without enabling bayer conversion",
		method);
    else {
...
// 	BayerMethod_ = DC1394_BAYER_METHOD_EDGESENSE;
	...
	ROS_ERROR("unknown bayer method [%s].  Defaulting to DownSample.",
                  method);
      }
    }
  }
		     ...
		     const char* method)
{
  ...
  if (d == NULL)
    {
      CAM_EXCEPT(bumblebee2::Exception,
  ...
  for (unsigned i=0; i < list->num; i++)
	...
	ROS_WARN_STREAM("Failed to initialize camera with GUID "
      ...
      else ROS_INFO_STREAM("Found camera with GUID "
                           << std::hex << list->ids[i].guid);
  ...
  if (!camera_)
    {
      if (strcmp(guid,"NONE")==0)
        ...
        {
          CAM_EXCEPT_ARGS(bumblebee2::Exception,
...
int Camera1394::setWhiteBalance(const char* whitebalance)
{
  ...
  if (setWhiteBalance)
                                   ...
                                   DC1394_FEATURE_MODE_MANUAL)))
	{
      ...
      if (!autoWhiteBalance)
	if (DC1394_SUCCESS !=
            dc1394_feature_whitebalance_set_value(camera_, blueBalance,
                                                  redBalance))
	  {
  ...
  if (DoBayerConversion_)
    {
      ...
      frame2.image =
        (unsigned char *) malloc(frame->size[0] * frame->size[1]
...
void Camera1394::uyvy2rgb (unsigned char *src, unsigned char *dest,
